...
<?php $env_code = $this->settings->get_environment(); $env_label = \Sii\BoletaDte\Infrastructure\WordPress\Settings::environment_label( $env_code ); ?>
<h1 style="display:flex;align-items:center;gap:12px;">
        <?php echo esc_html__( 'Panel de control', 'sii-boleta-dte' ); ?>
        <span style="background:#e9f2ff;color:#083e6d;padding:6px 10px;border-radius:999px;font-size:13px;box-shadow:0 1px 0 rgba(0,0,0,0.04);">
                <?php echo esc_html( $env_label ); ?> <small style="opacity:0.8;margin-left:6px;">(<?php echo esc_html( $env_code ); ?>)</small>
        </span>
</h1>
...
<h2 class="nav-tab-wrapper" id="sii-control-panel-tabs">
								<?php
...
<th><?php echo esc_html__( 'Disponibles', 'sii-boleta-dte' ); ?></th>
	...
	/** Shows latest log entries. */
        private function render_recent_logs(): void {
                ...
                // Para agrupar por Track ID y paginar por grupos, obtenemos un
                // número amplio de filas crudas y calculamos la paginación
                // sobre los grupos en esta capa de presentación.
                ...
                // If there are no rows for the configured environment and the
                // user hasn't applied any filters, it's likely that the DB
                // contains entries from a different environment (e.g. 'dev').
                // In that case, fall back to a query without environment so
                // administrators can still see existing log entries.
                $has_filters = ( $status !== '' ) || ( null !== $type ) || ( $date_from !== '' ) || ( $date_to !== '' );
                if ( 0 === (int) $logs_data['total'] && ! $has_filters ) {
                        $logs_data = LogDb::get_logs_paginated(
                                array(
                                        'limit'       => $per_page,
                                        'page'        => $page,
                ...
                $rows       = $logs_data['rows'];
                // Construir grupos por Track ID y calcular paginación por grupos
                $groups = array();
                $no_tid = 0;
                foreach ( (array) $rows as $r ) {
                        $tid = isset( $r['track_id'] ) ? trim( (string) $r['track_id'] ) : '';
                        if ( '' === $tid ) { $tid = '__NO_TRACK__' . (++$no_tid); }
                        if ( ! isset( $groups[ $tid ] ) ) { $groups[ $tid ] = array(); }
                        $groups[ $tid ][] = $r;
                }
                // Ordenar cada historial por fecha desc y los grupos por fecha desc del último
                ...
                foreach ( $groups as $tid => $items ) {
                        ...
                        $ordered_groups[] = array( 'tid' => $tid, 'items' => $items, 'latest_ts' => isset( $items[0]['created_at'] ) ? strtotime( (string) $items[0]['created_at'] ) : 0 );
                }
                usort( $ordered_groups, static function( $a, $b ){
                        return ( $b['latest_ts'] ?? 0 ) <=> ( $a['latest_ts'] ?? 0 );
                });

                $group_total = count( $ordered_groups );
                $limit_used  = $per_page;
                $total_pages = $limit_used > 0 ? max( 1, (int) ceil( $group_total / $limit_used ) ) : 1;
                $current_page = min( max( 1, (int) $page ), (int) $total_pages );
                $offset = ( $current_page - 1 ) * $limit_used;
                $paged_groups = array_slice( $ordered_groups, $offset, $limit_used );
                $start_index = $group_total > 0 ? $offset + 1 : 0;
                $end_index   = $group_total > 0 ? min( $offset + count( $paged_groups ), $group_total ) : 0;
                ...
                $available_statuses = array(
                        ...
                        'cancelled' => __( 'Cancelado', 'sii-boleta-dte' ),
		...
		$has_rows     = ! empty( $rows );
                $summary_text = $group_total > 0
                        ? sprintf( __( 'Mostrando %1$d-%2$d de %3$d registros', 'sii-boleta-dte' ), $start_index, $end_index, $group_total )
                        : '';
        ...
        <div class="sii-section">
                ...
                <?php $logs_base = function_exists( 'menu_page_url' ) ? \call_user_func( 'menu_page_url', 'sii-boleta-dte', false ) : '?page=sii-boleta-dte'; ?>
                <form class="sii-log-filters" method="get" action="<?php echo esc_url( $logs_base ); ?>">
                        <input type="hidden" name="page" value="sii-boleta-dte" />
                        <input type="hidden" name="tab" value="logs" />
                        <div class="sii-log-filters-grid">
                                <div class="sii-log-filter-row">
                                        <label for="logs_status"><?php echo esc_html__( 'Estado', 'sii-boleta-dte' ); ?></label>
                                        <select id="logs_status" name="logs_status">
                                ...
                                <div class="sii-log-filter-row">
                                        <label for="logs_type"><?php echo esc_html__( 'Tipo DTE', 'sii-boleta-dte' ); ?></label>
                                        <select id="logs_type" name="logs_type">
                                ...
                                <div class="sii-log-filter-row">
                                        <label for="logs_from"><?php echo esc_html__( 'Desde', 'sii-boleta-dte' ); ?></label>
                                        <input type="date" id="logs_from" name="logs_from" value="<?php echo esc_attr( $date_from ); ?>" />
                                </div>
                                <div class="sii-log-filter-row">
                                        <label for="logs_to"><?php echo esc_html__( 'Hasta', 'sii-boleta-dte' ); ?></label>
                                        <input type="date" id="logs_to" name="logs_to" value="<?php echo esc_attr( $date_to ); ?>" />
                                </div>
                                <div class="sii-log-filter-row">
                                        <label for="logs_per_page"><?php echo esc_html__( 'Por página', 'sii-boleta-dte' ); ?></label>
                                        <select id="logs_per_page" name="logs_per_page">
                                                <?php foreach ( array(10,25,50) as $opt ) : ?>
                                                        <option value="<?php echo (int) $opt; ?>"<?php selected( (int) $per_page, (int) $opt ); ?>><?php echo (int) $opt; ?></option>
                                                <?php endforeach; ?>
                                        </select>
                                </div>
                        </div>
                        <div class="sii-log-filter-actions">
                                <button type="submit" class="button button-primary"><?php echo esc_html__( 'Filtrar', 'sii-boleta-dte' ); ?></button>
                                <a class="button" href="<?php echo esc_url( $logs_base . '&tab=logs' ); ?>"><?php echo esc_html__( 'Limpiar', 'sii-boleta-dte' ); ?></a>
                        </div>
                </form>
		...
		<div class="sii-log-toolbar">
			<div id="log-summary" class="sii-log-summary" data-total="<?php echo (int) $total_rows; ?>" data-page="<?php echo (int) $current_page; ?>" data-pages="<?php echo (int) $total_pages; ?>" data-limit="<?php echo (int) $limit_used; ?>">
				...
				<?php endif; ?>
                        ...
                        <div id="log-pagination" class="sii-log-pagination" data-page="<?php echo (int) $current_page; ?>" data-pages="<?php echo (int) $total_pages; ?>" data-total="<?php echo (int) $group_total; ?>" data-limit="<?php echo (int) $limit_used; ?>">
                                <?php
                                $logs_base = function_exists( 'menu_page_url' ) ? \call_user_func( 'menu_page_url', 'sii-boleta-dte', false ) : '?page=sii-boleta-dte';
                                $query_args = array(
                                        'page'        => 'sii-boleta-dte',
                                        'tab'         => 'logs',
                                        'logs_status' => $status,
                                        'logs_type'   => ( null !== $type ? (string) $type : '' ),
                                        'logs_from'   => $date_from,
                                        'logs_to'     => $date_to,
                                        'logs_per_page' => (string) $limit_used,
                                );
                                $prev_url = $current_page > 1 ? add_query_arg( array_merge( $query_args, array( 'logs_page' => $current_page - 1 ) ), $logs_base ) : '#';
                                $next_url = $current_page < $total_pages ? add_query_arg( array_merge( $query_args, array( 'logs_page' => $current_page + 1 ) ), $logs_base ) : '#';
                                ?>
                                <a class="button<?php echo $current_page <= 1 ? ' disabled' : ''; ?>" href="<?php echo esc_url( $prev_url ); ?>"<?php echo $current_page <= 1 ? ' aria-disabled="true"' : ''; ?>><?php echo esc_html__( 'Anterior', 'sii-boleta-dte' ); ?></a>
                                <span><?php echo esc_html( sprintf( __( 'Página %1$d de %2$d', 'sii-boleta-dte' ), max( 1, $current_page ), max( 1, $total_pages ) ) ); ?></span>
                                <a class="button<?php echo $current_page >= $total_pages ? ' disabled' : ''; ?>" href="<?php echo esc_url( $next_url ); ?>"<?php echo $current_page >= $total_pages ? ' aria-disabled="true"' : ''; ?>><?php echo esc_html__( 'Siguiente', 'sii-boleta-dte' ); ?></a>
                        </div>
                </div>

		<div id="sii-log-empty-state" style="<?php echo $has_rows ? 'display:none;' : ''; ?>">
			<div style="padding: 20px; text-align: center;">
				...
				<?php else : ?>
					<p style="color: #666; font-size: 14px;">
						<?php echo esc_html__( 'Cuando generes o envíes DTE aparecerán aquí automáticamente.', 'sii-boleta-dte' ); ?>
					</p>
				<?php endif; ?>
			</div>
		</div>

		<div class="sii-log-table-wrapper" style="<?php echo $has_rows ? '' : 'display:none;'; ?>">
			<table class="widefat striped" id="sii-control-logs-table">
                ...
                <tbody id="sii-control-logs-body">
                    <?php
                    // Agrupar por Track ID y mostrar resumen + historial (desc)
                    $groups = array();
                    $no_tid = 0;
                    foreach ( (array) $rows as $r ) {
                        $tid = isset( $r['track_id'] ) ? trim( (string) $r['track_id'] ) : '';
                    ...
                    foreach ( $groups as $tid => $items ) :
                        ...
                        $latest   = $items[0];
                        $track_id_value = (string) ( $latest['track_id'] ?? '' );
                        $latest_type  = isset( $latest['document_type'] ) ? (int) $latest['document_type'] : 0;
                        $latest_folio = isset( $latest['folio'] ) ? (int) $latest['folio'] : 0;
                        $latest_status= (string) ( $latest['status'] ?? '' );
                        $latest_date  = (string) ( $latest['created_at'] ?? '' );
                        $group_id = 'loggrp-' . md5( $tid . '-' . (++$gidx) );
                    ?>
                        <tr class="sii-log-group-toggle" data-group="<?php echo esc_attr( $group_id ); ?>" tabindex="0" role="button" aria-expanded="false">
                            <td>
                                <?php if ( 0 !== strpos( $tid, '__NO_TRACK__' ) && '' !== $track_id_value ) : ?>
                                <span class="sii-track-id-clickable" 
                                    ...
                                    title="<?php echo esc_attr__( 'Clic para consultar estado en el SII', 'sii-boleta-dte' ); ?>"
                            ...
                            <td><?php echo esc_html( $this->dte_type_label( $latest_type ) ); ?></td>
                            <td><?php echo $latest_folio > 0 ? (int) $latest_folio : '-'; ?></td>
                            <td><?php echo esc_html( $this->translate_status_label( $latest_status ) ); ?></td>
                            <td><?php echo esc_html( $this->format_log_date( $latest_date ) ); ?></td>
                        </tr>
                        <tr id="<?php echo esc_attr( $group_id ); ?>" class="sii-log-group-details" style="display:none;">
                            <td colspan="5">
                                <table class="widefat striped" style="margin:8px 0;">
                                    ...
                                    <tbody>
                                        <?php foreach ( $items as $it ) : ?>
                                            <tr>
                                                <td><?php echo esc_html( $this->translate_status_label( (string) ( $it['status'] ?? '' ) ) ); ?></td>
                                                <td><?php echo isset( $it['folio'] ) && (int) $it['folio'] > 0 ? (int) $it['folio'] : '-'; ?></td>
                                                <td><?php echo esc_html( $this->format_log_date( (string) ( $it['created_at'] ?? '' ) ) ); ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
...
(function() {
    'use strict';
			...
			if (!modal) return;                        var overlay = modal.querySelector('.sii-modal-overlay');
                        ...
                        function openModal() {
                                // CSS expects the modal container to be flex for centering
        ...
        // Fallback robusto: alternar historial de logs por Track ID
        document.addEventListener('click', function (e) {
                var insideTrackLink = e.target.closest('.sii-track-id-clickable');
                if (insideTrackLink) { return; }
                var row = e.target.closest('tr.sii-log-group-toggle');
                if (!row) { return; }
                var id = row.getAttribute('data-group');
                if (!id) { return; }
                var detail = document.getElementById(id);
                if (!detail) { return; }
                var isHidden = (detail.style.display === 'none' || window.getComputedStyle(detail).display === 'none');
        ...
        document.addEventListener('keydown', function (e) {
                ...
                row.setAttribute('aria-expanded', isHidden ? 'true' : 'false');
                        ...
                        // Click or keyboard activation on Track ID (single click)
                        document.addEventListener('click', function(e) {
                                var target = e.target;
                                if (!target.classList.contains('sii-track-id-clickable')) return;

                                triggerTrackQuery(target);
                        });

                        // Click on "Detalles" buttons (show movements)
                        document.addEventListener('click', function(e) {
                                var target = e.target;
                                if (!target.classList.contains('sii-track-details-btn')) return;
                                // Reuse modal UI
                                        ...
                                        if (xhr.status === 200) {
                                                try {
                                                        ...
                                                        if (response.success) {
                                                                ...
                                                                showResult('<div class="notice notice-error"><p>' + (response.data && response.data.message ? response.data.message : ERROR_MESSAGES.queryStatus) + '</p></div>');
                                                        }
                                                } catch (e) {
                                ...
                                xhr.onerror = function() { showResult('<div class="notice notice-error"><p>' + ERROR_MESSAGES.network + '</p></div>'); };
                                xhr.send('action=sii_boleta_query_track_status&track_id=' + encodeURIComponent(trackId) + '&include_movements=1&nonce=' + encodeURIComponent(NONCE));
                        });

                        // Allow Enter key to trigger when focused
                        document.addEventListener('keydown', function(e) {
                                if (e.key !== 'Enter') return;
                                var target = e.target;
                                if (!target.classList || !target.classList.contains('sii-track-id-clickable')) return;
                                e.preventDefault();
                                triggerTrackQuery(target);
                        });

                        // Shared logic to query and render
                        function triggerTrackQuery(target) {
                                var trackId = target.getAttribute('data-track-id');
                                                ...
                                                try {
                                                        ...
                                                        if (response.success) {
                                                                ...
                                                                showResult('<div class="notice notice-error"><p>' + 
                                                                        (response.data && response.data.message ? response.data.message : ERROR_MESSAGES.queryStatus) + 
                                                                        '</p></div>');
                                                        }
                                                } catch (e) {
                                                        showResult('<div class="notice notice-error"><p>' + ERROR_MESSAGES.processResponse + '</p></div>');
...
private function render_queue(): void {
		$environment = $this->settings->get_environment();
            ...
            // Estado del cron de cola
            $cron_disabled = defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON;
            $next_hook_ts = 0;
            if ( function_exists( 'wp_next_scheduled' ) ) {
                    $next_hook_ts = (int) wp_next_scheduled( \Sii\BoletaDte\Infrastructure\Scheduling\Cron::HOOK );
            }
		
            // Obtener trabajos disponibles para ejecutar ahora
            // (created_at <= NOW()) y, además, incluir los programados para
            // el futuro para que el panel sea consistente con el contador.
            $raw_jobs = QueueDb::get_pending_jobs( 300 );

            $show_scheduled_hint = false;
            $scheduled_count      = 0;

            // Incorporar también los programados a futuro (por reintentos)
            // para que el administrador pueda verlos en la lista aunque aún
            // no estén disponibles para ejecución.
            $all_db_jobs = QueueDb::get_all_jobs( 300 );
            if ( is_array( $all_db_jobs ) && count( $all_db_jobs ) > 0 ) {
                    $now_ts = function_exists( 'current_time' ) ? strtotime( current_time( 'mysql', true ) ) : time();
                    $future_jobs = array_filter(
                            $all_db_jobs,
                            static function ( array $j ) use ( $now_ts ) {
                                    $created = isset( $j['created_at'] ) ? strtotime( (string) $j['created_at'] ) : 0;
                                    return $created > $now_ts;
                            }
                    );
                    if ( ! empty( $future_jobs ) ) {
                            // Mezclar manteniendo IDs únicos para evitar duplicados
                            $by_id = array();
                            foreach ( $raw_jobs as $j ) { $by_id[ (int) ( $j['id'] ?? 0 ) ] = $j; }
                            foreach ( $future_jobs as $j ) { $by_id[ (int) ( $j['id'] ?? 0 ) ] = $j; }
                            $raw_jobs           = array_values( $by_id );
                            $show_scheduled_hint = true;
                            $scheduled_count     = count( $future_jobs );
                    }
            }
	...
	if ( ! empty( $_GET['filter_to'] ) ) {
        ...
        // Paginación de cola (GET: queue_page, queue_per_page)
        $queue_page_raw = isset( $_GET['queue_page'] ) ? (string) $_GET['queue_page'] : '';
        $queue_page = '' === $queue_page_raw ? 1 : max( 1, (int) $queue_page_raw );
        $queue_pp_raw = isset( $_GET['queue_per_page'] ) ? (string) $_GET['queue_per_page'] : '';
        $queue_per_page = '' === $queue_pp_raw ? 10 : max( 5, min( 100, (int) $queue_pp_raw ) );
        $queue_total = count( $jobs );
        $queue_pages = $queue_per_page > 0 ? (int) max( 1, ceil( $queue_total / $queue_per_page ) ) : 1;
        if ( $queue_page > $queue_pages ) { $queue_page = $queue_pages; }
        $queue_offset = ( $queue_page - 1 ) * $queue_per_page;
        $paged_jobs = array_slice( $jobs, $queue_offset, $queue_per_page );
        ...
        <div class="sii-section">
                ...
                <div style="background:#eef6ff;padding:10px 12px;border-radius:6px;margin:10px 0;display:flex;gap:16px;align-items:center;flex-wrap:wrap;">
                        <div>
                                ...
                                <?php echo $cron_disabled ? '<span style="color:#d63638;">' . esc_html__( 'Deshabilitado (DISABLE_WP_CRON)', 'sii-boleta-dte' ) . '</span>' : esc_html__( 'Habilitado', 'sii-boleta-dte' ); ?>
                        </div>
                        <div>
                                <strong><?php echo esc_html__( 'Próxima ejecución:', 'sii-boleta-dte' ); ?></strong>
                                <?php echo $next_hook_ts > 0 ? esc_html( $this->format_datetime( $next_hook_ts ) ) : '<em>' . esc_html__( 'No programada', 'sii-boleta-dte' ) . '</em>'; ?>
                        </div>
                        ...
                        <div>
                                <form method="post" style="margin:0;display:flex;gap:8px;align-items:center;">
                                        ...
                                        <input type="hidden" name="schedule_queue" value="1" />
                                        <?php $current_interval = $this->settings->get_queue_interval(); ?>
                                        <label for="queue_interval" style="margin-right:4px;">{
                                                <?php echo esc_html__( 'Frecuencia:', 'sii-boleta-dte' ); ?>
                                        }</label>
                                        <select name="queue_interval" id="queue_interval">
                                                <option value="every_minute" <?php selected( $current_interval, 'every_minute' ); ?>><?php echo esc_html__( 'Cada 1 minuto', 'sii-boleta-dte' ); ?></option>
                                                <option value="every_five_minutes" <?php selected( $current_interval, 'every_five_minutes' ); ?>><?php echo esc_html__( 'Cada 5 minutos', 'sii-boleta-dte' ); ?></option>
                        ...
                        <div>
                                <form method="post" style="margin:0;display:inline;">
                                        ...
                                        <button type="submit" class="button"><?php echo esc_html__( 'Desprogramar', 'sii-boleta-dte' ); ?></button>
                                </form>
                        </div>
                        <?php endif; ?>
                </div>
                ...
                <!-- Filtros -->
                <?php if ( ! empty( $show_scheduled_hint ) ) : ?>
                        <div style="background: #fff8e1; border-left: 4px solid #ffc107; padding: 12px; margin: 12px 0; border-radius: 4px;">
                                <strong>📅 <?php echo esc_html__( 'Trabajos programados', 'sii-boleta-dte' ); ?></strong>
                                <div style="margin-top:6px; color:#856404;">
                                        <?php echo sprintf( esc_html__( 'Se han detectado %d trabajo(s) programado(s) para ejecutarse en el futuro. Estos no aparecen como pendientes hasta su fecha de ejecución.', 'sii-boleta-dte' ), (int) $scheduled_count ); ?>
                                </div>
                        </div>
                <?php endif; ?>
                <div id="queue-filters" style="background: #f9f9f9; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
                        <?php $queue_base = function_exists( 'menu_page_url' ) ? \call_user_func( 'menu_page_url', 'sii-boleta-dte', false ) : '?page=sii-boleta-dte'; ?>
                        <form id="queue-filter-form" method="get" action="<?php echo esc_url( $queue_base ); ?>" style="display: flex; gap: 10px; align-items: end; flex-wrap: wrap;">
                                <input type="hidden" name="page" value="sii-boleta-dte" />
                                ...
                                <div>
                                        ...
                                        <select name="filter_attempts" id="filter_attempts" class="queue-filter">
                                                <?php $att = isset( $filters['attempts'] ) ? (string) $filters['attempts'] : ''; ?>
                                                <option value="" <?php selected( $att, '' ); ?>><?php echo esc_html__( 'Todos', 'sii-boleta-dte' ); ?></option>
                                                <option value="0" <?php selected( $att, '0' ); ?>><?php echo esc_html__( 'Sin intentos (0)', 'sii-boleta-dte' ); ?></option>
                                                <option value="1" <?php selected( $att, '1' ); ?>><?php echo esc_html__( '1 intento', 'sii-boleta-dte' ); ?></option>
                                                <option value="2" <?php selected( $att, '2' ); ?>><?php echo esc_html__( '2 intentos', 'sii-boleta-dte' ); ?></option>
                                                <option value="3+" <?php selected( $att, '3+' ); ?>><?php echo esc_html__( '3+ intentos (fallidos)', 'sii-boleta-dte' ); ?></option>
                                        </select>
                                </div>
				...
				<div>
					...
					<select name="filter_age" id="filter_age" class="queue-filter">
                                <?php $agev = isset( $filters['age'] ) ? (string) $filters['age'] : ''; ?>
						<option value="" <?php selected( $agev, '' ); ?>><?php echo esc_html__( 'Todos', 'sii-boleta-dte' ); ?></option>
						<option value="new" <?php selected( $agev, 'new' ); ?>><?php echo esc_html__( 'Recientes (<1h)', 'sii-boleta-dte' ); ?></option>
						<option value="old" <?php selected( $agev, 'old' ); ?>><?php echo esc_html__( 'Antiguos (>1h)', 'sii-boleta-dte' ); ?></option>
					</select>
				</div>
				<div>
					...
					<input type="date" name="filter_from" id="filter_from" class="queue-filter" value="<?php echo isset( $filters['from'] ) ? esc_attr( (string) $filters['from'] ) : ''; ?>" />
				</div>
				<div>
					<label for="filter_to" style="display: block; font-size: 12px; margin-bottom: 3px;">
						<?php echo esc_html__( 'Hasta (fecha):', 'sii-boleta-dte' ); ?>
					</label>
					<input type="date" name="filter_to" id="filter_to" class="queue-filter" value="<?php echo isset( $filters['to'] ) ? esc_attr( (string) $filters['to'] ) : ''; ?>" />
				</div>
				<div>
					<label for="queue_per_page" style="display: block; font-size: 12px; margin-bottom: 3px;">
						<?php echo esc_html__( 'Por página', 'sii-boleta-dte' ); ?>
					</label>
					<select name="queue_per_page" id="queue_per_page" class="queue-filter">
						<?php foreach ( array(10,25,50) as $opt ) : ?>
							<option value="<?php echo (int) $opt; ?>"<?php selected( (int) $queue_per_page, (int) $opt ); ?>><?php echo (int) $opt; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div>
					...
					<a class="button" href="<?php echo esc_url( $queue_base . '&tab=queue' ); ?>"><?php echo esc_html__( 'Limpiar', 'sii-boleta-dte' ); ?></a>
	...
	<?php endif; ?>
                ...
                <?php
                        ...
                        $next_queue_ts = 0;
                        if ( function_exists( 'wp_next_scheduled' ) ) {
                                $hook = \Sii\BoletaDte\Infrastructure\Scheduling\Cron::HOOK;
                                $ts   = wp_next_scheduled( $hook );
                                if ( is_int( $ts ) && $ts > 0 ) { $next_queue_ts = $ts; }
                        }
                ?>
                <div id="sii-queue-help" style="display: none; background: #e7f3ff; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
                        ...
                        <ul>
                                <li><strong><?php echo esc_html__( 'Procesamiento:', 'sii-boleta-dte' ); ?></strong> <?php echo esc_html__( 'Cada 5 minutos (cron de WordPress).', 'sii-boleta-dte' ); ?></li>
                                <li><strong><?php echo esc_html__( 'Próxima ejecución:', 'sii-boleta-dte' ); ?></strong> <?php echo esc_html( $this->format_datetime( (int) $next_queue_ts ) ); ?></li>
                                <li><strong><?php echo esc_html__( 'Estado del cron:', 'sii-boleta-dte' ); ?></strong> <?php echo $cron_disabled ? '<span style="color:#d63638;">' . esc_html__( 'Deshabilitado', 'sii-boleta-dte' ) . '</span>' : esc_html__( 'Habilitado', 'sii-boleta-dte' ); ?></li>
                                <li><strong><?php echo esc_html__( 'Reintentos:', 'sii-boleta-dte' ); ?></strong> <?php echo esc_html__( 'Hasta 3 intentos con 2 minutos de espera. Luego queda pausado para acción manual.', 'sii-boleta-dte' ); ?></li>
                                ...
                                <li><strong><?php echo esc_html__( 'Trabajos programados:', 'sii-boleta-dte' ); ?></strong> <?php echo esc_html__( 'Los reintentos se muestran aunque estén programados para el futuro.', 'sii-boleta-dte' ); ?></li>
                        ...
                        <?php if ( $cron_disabled ) : ?>
                                <p style="margin-top:10px;color:#d63638;">⚠ <?php echo esc_html__( 'El cron de WordPress está deshabilitado (DISABLE_WP_CRON). Programa una tarea del sistema para ejecutar wp-cron.php cada 5 minutos.', 'sii-boleta-dte' ); ?></p>
                        <?php endif; ?>
                </div>
	        ...
	        <div class="sii-queue-empty-card">
                ...
                <?php
                        ...
                        $query_args = array(
                                'page'            => 'sii-boleta-dte',
                                'tab'             => 'queue',
                                'filter_attempts' => isset( $filters['attempts'] ) ? (string) $filters['attempts'] : '',
                                'filter_age'      => isset( $filters['age'] ) ? (string) $filters['age'] : '',
                                'filter_from'     => isset( $filters['from'] ) ? (string) $filters['from'] : '',
                                'filter_to'       => isset( $filters['to'] ) ? (string) $filters['to'] : '',
                                'queue_per_page'  => (string) $queue_per_page,
                        );
                        $prev_q = $query_args; $prev_q['queue_page'] = max( 1, $queue_page - 1 );
                        $next_q = $query_args; $next_q['queue_page'] = min( $queue_pages, $queue_page + 1 );
                        $prev_url = add_query_arg( $prev_q, $queue_base );
                        $next_url = add_query_arg( $next_q, $queue_base );
                        $q_start = $queue_total > 0 ? ( $queue_offset + 1 ) : 0;
                        $q_end   = $queue_total > 0 ? min( $queue_offset + count( $paged_jobs ), $queue_total ) : 0;
                ?>
                <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin:6px 0 8px;">
                        <div class="sii-queue-summary">
                                <?php if ( $queue_total > 0 ) : ?>
                                        <span><?php echo esc_html( sprintf( __( 'Mostrando %1$d-%2$d de %3$d trabajos', 'sii-boleta-dte' ), (int) $q_start, (int) $q_end, (int) $queue_total ) ); ?></span>
                                <?php endif; ?>
                        </div>
                        <div>
                                <a class="button<?php echo $queue_page <= 1 ? ' disabled' : ''; ?>" href="<?php echo esc_url( $prev_url ); ?>"<?php echo $queue_page <= 1 ? ' aria-disabled=\"true\"' : ''; ?>><?php echo esc_html__( 'Anterior', 'sii-boleta-dte' ); ?></a>
                                <span><?php echo esc_html( sprintf( __( 'Página %1$d de %2$d', 'sii-boleta-dte' ), max(1,$queue_page), max(1,$queue_pages) ) ); ?></span>
                                <a class="button<?php echo $queue_page >= $queue_pages ? ' disabled' : ''; ?>" href="<?php echo esc_url( $next_url ); ?>"<?php echo $queue_page >= $queue_pages ? ' aria-disabled=\"true\"' : ''; ?>><?php echo esc_html__( 'Siguiente', 'sii-boleta-dte' ); ?></a>
                        </div>
                </div>
...
<?php 
    ...
    // Determinar si el trabajo está programado para el futuro
    $created_raw = isset($job['created_at']) ? (string)$job['created_at'] : (isset($job['available_at']) ? (string)$job['available_at'] : '');
    $now_ts     = function_exists('current_time') ? strtotime( current_time( 'mysql', true ) ) : time();
    $created_ts = $created_raw ? strtotime( $created_raw ) : false;
    $is_scheduled = ( $created_ts && $created_ts > $now_ts );
	...
	<td><?php echo esc_html( $this->queue_job_type_label( $job ) ); ?></td>
	<td>
        ...
        <?php elseif ( $is_scheduled ) : ?>
                <br><small style="color:#856404;">
                        <?php echo esc_html__( 'Programado para reintento', 'sii-boleta-dte' ); ?>
                </small>
        <?php endif; ?>
</td>
<td>
	...
	<td>
        ...
        <form method="post" class="sii-inline-form" style="display:inline; margin:0; padding:0;">
...
<?php $this->output_nonce_field( 'sii_boleta_queue', 'sii_boleta_queue_nonce' ); ?>
            ...
            <button type="submit" name="queue_action" value="requeue" class="button sii-queue-action sii-queue-action-sm" title="<?php echo esc_attr__( 'Reintentar', 'sii-boleta-dte' ); ?>">⟳</button>
...
<div id="sii-queue-confirm-modal" class="sii-modal" style="display:none;">
        <div class="sii-modal-overlay" role="presentation"></div>
        <div class="sii-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="sii-queue-confirm-title">
                <div class="sii-modal-header">
                        <h3 id="sii-queue-confirm-title"><?php echo esc_html__( 'Confirmar acción', 'sii-boleta-dte' ); ?></h3>
                </div>
                <div class="sii-modal-body">
                        <p id="sii-queue-confirm-message"><?php echo esc_html__( 'Esta acción se ejecutará inmediatamente. ¿Deseas continuar?', 'sii-boleta-dte' ); ?></p>
                </div>
                <div class="sii-modal-footer" style="display:flex;gap:8px;justify-content:flex-end;">
                        <button type="button" class="button button-primary" id="sii-queue-confirm-accept"><?php echo esc_html__( 'Continuar', 'sii-boleta-dte' ); ?></button>
                        <button type="button" class="button" id="sii-queue-confirm-cancel"><?php echo esc_html__( 'Cancelar', 'sii-boleta-dte' ); ?></button>
                </div>
        </div>
</div>
...
<?php
    try {
        $rvd_summary = method_exists( $this->rvd_manager, 'get_daily_summary' )
            ? $this->rvd_manager->get_daily_summary()
            : array();
    } catch ( \Throwable $e ) {
        $rvd_summary = array();
    }
    if ( ! is_array( $rvd_summary ) ) {
        $rvd_summary = array();
    }
    $rvd_summary = array_merge(
        array( 'date' => gmdate( 'Y-m-d' ), 'documents' => 0, 'net' => 0, 'iva' => 0, 'total' => 0, 'details' => array() ),
        $rvd_summary
    );
	$rvd_has_data = ( (int) ( $rvd_summary['documents'] ?? 0 ) ) > 0 || ( (int) ( $rvd_summary['total'] ?? 0 ) ) > 0;
	$rvd_details = isset( $rvd_summary['details'] ) && is_array( $rvd_summary['details'] ) ? $rvd_summary['details'] : array();
?>
<div class="sii-table-wrapper" style="margin-bottom:16px;">
	<table class="widefat striped sii-rvd-summary-table">
		<thead>
			<tr>
				<th><?php echo esc_html__( 'Fecha', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Documentos', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Monto Neto', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'IVA', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Monto Total', 'sii-boleta-dte' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php if ( $rvd_has_data ) : ?>
				<tr class="sii-rvd-summary-row<?php echo ! empty( $rvd_details ) ? ' has-details' : ''; ?>"<?php echo ! empty( $rvd_details ) ? ' tabindex="0" role="button" aria-expanded="false" aria-controls="sii-rvd-details"' : ''; ?>>
					<td><?php echo esc_html( (string) ( $rvd_summary['date'] ?? '' ) ); ?></td>
					<td><?php echo esc_html( (string) (int) ( $rvd_summary['documents'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $rvd_summary['net'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $rvd_summary['iva'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $rvd_summary['total'] ?? 0 ) ); ?></td>
				</tr>
			<?php else : ?>
				<tr>
					<td colspan="5"><?php echo esc_html__( 'No se registran ventas para el período actual. Cuando existan montos, aparecerán aquí antes de enviarse al SII.', 'sii-boleta-dte' ); ?></td>
				</tr>
			<?php endif; ?>
		</tbody>
	</table>
</div>
<?php if ( $rvd_has_data && ! empty( $rvd_details ) ) : ?>
	<p class="description" id="sii-rvd-detail-hint"><?php echo esc_html__( 'Haz clic en la fila para ver el detalle de órdenes incluidas en el resumen.', 'sii-boleta-dte' ); ?></p>
	<div id="sii-rvd-details" class="sii-rvd-details" style="display:none;">
		<h3><?php echo esc_html__( 'Detalle de órdenes', 'sii-boleta-dte' ); ?></h3>
		<table class="widefat striped">
			<thead>
				<tr>
					<th><?php echo esc_html__( 'Fecha', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Orden', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Estado', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Tipo DTE', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Monto Neto', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'IVA', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Monto Total', 'sii-boleta-dte' ); ?></th>
					<th><?php echo esc_html__( 'Cliente', 'sii-boleta-dte' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ( $rvd_details as $detail ) : ?>
					<?php
					$order_id      = isset( $detail['id'] ) ? (int) $detail['id'] : 0;
					$order_number  = isset( $detail['number'] ) ? (string) $detail['number'] : (string) $order_id;
					$order_number  = '' === $order_number ? (string) $order_id : $order_number;
					$order_display = '#' . ltrim( $order_number, '#' );
					$order_link    = ( $order_id > 0 && function_exists( 'get_edit_post_link' ) ) ? get_edit_post_link( $order_id ) : '';
					$status_label  = $this->format_order_status( isset( $detail['status'] ) ? (string) $detail['status'] : '' );
					$doc_type      = isset( $detail['document_type'] ) ? (int) $detail['document_type'] : 0;
					$doc_label     = $doc_type > 0 ? $this->dte_type_label( $doc_type ) : '-';
					$created       = isset( $detail['created_at'] ) ? (string) $detail['created_at'] : '';
					?>
					<tr>
						...
						<td>
							<?php if ( $order_link ) : ?>
								<a href="<?php echo esc_url( $order_link ); ?>">
									<?php echo esc_html( $order_display ); ?>
								</a>
							<?php else : ?>
								<?php echo esc_html( $order_display ); ?>
							<?php endif; ?>
						...
						<td><?php echo esc_html( $this->format_amount( $detail['net'] ?? 0 ) ); ?></td>
						<td><?php echo esc_html( $this->format_amount( $detail['tax'] ?? 0 ) ); ?></td>
						<td><?php echo esc_html( $this->format_amount( $detail['total'] ?? 0 ) ); ?></td>
						<td><?php echo esc_html( (string) ( $detail['customer'] ?? '' ) ); ?></td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
	...
	<style>
	.sii-rvd-summary-row.has-details { cursor: pointer; }
	.sii-rvd-summary-row.has-details:focus { outline: 2px solid #2271b1; }
	</style>
<?php endif; ?>
<form method="post">
...
<?php
	$libro_summary = method_exists( $this->libro_boletas, 'get_monthly_summary' )
		? $this->libro_boletas->get_monthly_summary( $period )
		: array( 'period' => $period, 'documents' => 0, 'net' => 0, 'iva' => 0, 'total' => 0 );
	$libro_has_data = ( (int) ( $libro_summary['documents'] ?? 0 ) ) > 0 || ( (int) ( $libro_summary['total'] ?? 0 ) ) > 0;
?>
<div class="sii-table-wrapper" style="margin-bottom:16px;">
	<table class="widefat striped">
		<thead>
			<tr>
				<th><?php echo esc_html__( 'Período', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Documentos', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Monto Neto', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'IVA', 'sii-boleta-dte' ); ?></th>
				<th><?php echo esc_html__( 'Monto Total', 'sii-boleta-dte' ); ?></th>
		...
		<tbody>
			<?php if ( $libro_has_data ) : ?>
				<tr>
					<td><?php echo esc_html( (string) ( $libro_summary['period'] ?? $period ) ); ?></td>
					<td><?php echo esc_html( (string) (int) ( $libro_summary['documents'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $libro_summary['net'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $libro_summary['iva'] ?? 0 ) ); ?></td>
					<td><?php echo esc_html( $this->format_amount( $libro_summary['total'] ?? 0 ) ); ?></td>
				</tr>
			<?php else : ?>
				<tr>
					<td colspan="5"><?php echo esc_html__( 'No se encontraron boletas emitidas en el período seleccionado. Cuando existan registros, aparecerán aquí antes de generar el libro.', 'sii-boleta-dte' ); ?></td>
				</tr>
			<?php endif; ?>
		</tbody>
	</table>
...
<li><?php echo esc_html__( 'Período a informar', 'sii-boleta-dte' ) . ': ' . esc_html( $libro_period ); ?></li>
...
private function handle_libro_action( string $action, string $xml ): void {
	...
	private function render_notices(): void {
		...
		if ( false === $notices_json ) {
			$notices_json = '[]';
		}

		// Renderiza siempre el contenedor que consume control-panel.js
		echo '<div id="sii-notices-data" data-notices="' . esc_attr( $notices_json ) . '" style="display:none;"></div>';

		if ( empty( $notices ) ) {
			// No hay notificaciones adicionales que mostrar
			$this->notices = array();
			return;
		}
	...
	private function format_order_status( string $status ): string {
		...
		if ( function_exists( 'wc_get_order_status_name' ) ) {
			...
			$label = wc_get_order_status_name( $key );
	...
	private function format_log_date( string $datetime ): string {
                // Defensive: ensure we only pass strings to strtotime. If an unexpected
                // type (array/object) arrives, avoid TypeError and return a safe
                // placeholder while logging the issue for diagnosis.
                if ( '' === $datetime ) {
                        return '—';
                }
                if ( ! is_string( $datetime ) ) {
                        // Try to coerce scalars; otherwise log and return raw placeholder.
                        if ( is_scalar( $datetime ) ) {
                                $datetime = (string) $datetime;
                        } else {
                                if ( function_exists( 'error_log' ) ) {
                                        error_log( '[sii-boleta] format_log_date: unexpected datetime type: ' . gettype( $datetime ) );
                                }
                                return '—';
                        }
                }
	...
	private function describe_queue_document( array $job ): string {
		$type          = isset( $job['type'] ) ? (string) $job['type'] : '';
		$document_type = $this->resolve_queue_job_document_type( $job );
		$folio         = $this->resolve_queue_job_folio( $job );
		$label         = $this->resolve_queue_job_label( $job );
		...
		if ( 'dte' === $type ) {
			...
			if ( '' !== $label ) {
				$parts[] = $label;
			} elseif ( null !== $document_type ) {
				$parts[] = $this->dte_type_label( $document_type );
			}
		} elseif ( '' !== $label ) {
			$parts[] = $label;
		}
	...
	private function queue_job_type_label( array $job ): string {
		$type = isset( $job['type'] ) ? (string) $job['type'] : '';
		if ( 'dte' !== $type ) {
			return $this->translate_type( $type );
		}
		...
		if ( null !== $document_type ) {
			return $this->dte_type_label( $document_type );
		}

		return $this->translate_type( 'dte' );
	}

	private function resolve_queue_job_document_type( array $job ): ?int {
		$payload = $this->queue_job_payload( $job );
		$meta    = $this->queue_job_meta( $payload );

		$type = $payload['document_type'] ?? ( $meta['type'] ?? null );
		if ( is_numeric( $type ) ) {
			$type = (int) $type;
	...
	private function resolve_queue_job_folio( array $job ): ?int {
		$payload = $this->queue_job_payload( $job );
		$meta    = $this->queue_job_meta( $payload );
		...
		if ( is_numeric( $folio ) ) {
			$folio = (int) $folio;
			return $folio > 0 ? $folio : null;
		}

		return null;
	}

	private function resolve_queue_job_label( array $job ): string {
		$payload = $this->queue_job_payload( $job );
		$meta    = $this->queue_job_meta( $payload );
	...
	private function queue_job_payload( array $job ): array {
		return isset( $job['payload'] ) && is_array( $job['payload'] ) ? $job['payload'] : array();
	}

	private function queue_job_meta( array $payload ): array {
		return isset( $payload['meta'] ) && is_array( $payload['meta'] ) ? $payload['meta'] : array();
	}
...
<p><?php echo esc_html__( 'Última ejecución:', 'sii-boleta-dte' ) . ' ' . esc_html( '' !== $last ? $last : __( 'Nunca', 'sii-boleta-dte' ) ); ?></p>
    ...
    private function handle_schedule_queue(): void {
        if ( ! $this->verify_nonce( 'sii_boleta_schedule_nonce', 'sii_boleta_schedule' ) ) {
            $this->add_notice( __( 'Token de seguridad inválido.', 'sii-boleta-dte' ), 'error' );
            return;
        }
        if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
            $this->add_notice( __( 'WP-Cron está deshabilitado (DISABLE_WP_CRON). Programa un cron del sistema para ejecutar wp-cron.php desde el sistema.', 'sii-boleta-dte' ), 'error' );
            return;
        }

        $unschedule = ! empty( $_POST['unschedule'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
        if ( $unschedule ) {
            try {
                \Sii\BoletaDte\Infrastructure\Scheduling\Cron::deactivate();
                $this->add_notice( __( 'Cron desprogramado.', 'sii-boleta-dte' ), 'success' );
            } catch ( \Throwable $e ) {
                $this->add_notice( __( 'No se pudo desprogramar el cron.', 'sii-boleta-dte' ), 'error' );
            }
            return;
        }

        $interval = isset( $_POST['queue_interval'] ) ? (string) $_POST['queue_interval'] : 'every_five_minutes'; // phpcs:ignore WordPress.Security.NonceVerification.Missing
        if ( ! in_array( $interval, array( 'every_minute', 'every_five_minutes', 'every_fifteen_minutes' ), true ) ) {
            $interval = 'every_five_minutes';
        }
        try {
            \Sii\BoletaDte\Infrastructure\WordPress\Settings::set_queue_interval( $interval );
        } catch ( \Throwable $e ) { /* ignore persist failure */ }

        try {
            \Sii\BoletaDte\Infrastructure\Scheduling\Cron::reschedule( $interval );
            $label = match ( $interval ) {
                'every_minute' => __( 'cada 1 minuto', 'sii-boleta-dte' ),
                'every_fifteen_minutes' => __( 'cada 15 minutos', 'sii-boleta-dte' ),
