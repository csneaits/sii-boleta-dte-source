 ...
 *
 * Note about mapping extra fields: we intentionally do NOT auto-hydrate
 * additional receptor attributes here. The Provider interface only receives an
 * int|string|ReceptorInterface, so when a scalar RUT is provided we have no
 * accompanying name/address/email fields to map. The correct place to populate
 * those values is the document payload itself (which we already do in the
 * preparation pipeline), not the Provider. We still prefer the official
 * ReceptorFactory to create the typed entity when a scalar is provided.
 */
