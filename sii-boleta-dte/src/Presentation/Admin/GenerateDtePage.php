...
class GenerateDtePage {
	...
	private ?array $cached_emitter_giros = null;
	...
	public function process_post( array $post ): array {
		...
		if ( in_array( $tipo, array( 56, 61, 111, 112 ), true ) ) {
			...
			if ( '' !== $ref_folio ) {
				$ref_doc_type = 0 === $ref_tipo ? 39 : $ref_tipo;
				$ref          = array(
					'TpoDocRef' => $ref_doc_type,
					'FolioRef'  => $ref_folio,
				);
				if ( '' !== $ref_fecha ) {
	...
	private function get_emitter_giros(): array {
		if ( null !== $this->cached_emitter_giros ) {
				return $this->cached_emitter_giros;
		}

			$settings = $this->settings->get_settings();
			$giros    = array();

		if ( ! empty( $settings['giros'] ) && is_array( $settings['giros'] ) ) {
		...
		if ( empty( $giros ) && ! empty( $settings['giro'] ) ) {
			...
			$this->cached_emitter_giros = array_values( array_unique( $giros ) );
