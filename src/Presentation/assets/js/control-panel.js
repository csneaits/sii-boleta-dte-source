(function (window, document) {
    ...
    function refreshDomRefs() {
        ...
        rvdDetails = document.getElementById('sii-rvd-details');
    ...
    function toggleRvdDetails(row) {
        if (!rvdDetails) {
            return;
        }
        var isVisible = window.getComputedStyle(rvdDetails).display !== 'none';
        if (isVisible) {
            rvdDetails.style.display = 'none';
            if (row) {
                row.setAttribute('aria-expanded', 'false');
            }
        } else {
            rvdDetails.style.display = '';
            if (row) {
    ...
    document.addEventListener('click', function (event) {
        var summaryRow = event.target.closest('.sii-rvd-summary-row.has-details');
        if (!summaryRow) {
            return;
        }
        event.preventDefault();
        toggleRvdDetails(summaryRow);
    }, false);
