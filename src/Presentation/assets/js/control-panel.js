(function (window, document) {
        ...
        fetch(ajaxUrl, {
            ...
            .then(function (payload) {
                ...
                try {
                    ...
                    console.error('[sii-boleta] Snapshot JSON parse error. Raw:', payload);
                    throw e;
                }
    ...
    // Registrar el botón pulsado para recuperar la acción
    document.addEventListener('click', function (e) {
        var btn = e.target && e.target.closest && e.target.closest('.sii-inline-form button[name="queue_action"]');
        if (btn && btn.form) {
            btn.form.setAttribute('data-sii-submit-action', btn.value || '');
        }
    }, false);
    ...
    document.addEventListener('submit', function (e) {
        ...
        if (e.submitter && e.submitter.name === 'queue_action') {
            queueAction = e.submitter.value || '';
        }
        if (!queueAction && form.getAttribute('data-sii-submit-action')) {
            queueAction = form.getAttribute('data-sii-submit-action') || '';
        }
        if (!queueAction) {
            queueAction = formData.get('queue_action') || '';
        }
        var jobId = formData.get('job_id');
        // Prefer el nonce incluido en el formulario; si por cualquier motivo
        // no está presente (contenido cargado por AJAX, plugins de caché, etc),
        // usar el nonce global inyectado por PHP para el canal de cola.
        ...
        fetch(ajaxUrl, {
            ...
            .finally(function () {
                ...
                try { form.removeAttribute('data-sii-submit-action'); } catch (ex) {}
                Array.prototype.forEach.call(buttons, function (button) {
