...
class FactoryBackedDocumentPreparationPipeline implements DocumentPreparationPipelineInterface {
    ...
    public function prepare(
        ...
        if (null === $tasa_iva && in_array($tipo, array(33, 39, 43, 46, 52, 56, 61), true)) {
            // Prioridad de tasa:
            // 1) Valor explícito en datos del documento (Encabezado.Totales.TasaIVA)
            // 2) Plantilla YAML del tipo
            // 3) Configuración del plugin (iva_rate)
            // 4) Fallback histórico 19.0
            $configured = isset($settings['iva_rate']) ? (float) $settings['iva_rate'] : null;
            if (null !== $configured && $configured >= 0 && $configured <= 100) {
                $tasa_iva = $configured;
            } else {
                $tasa_iva = 19.0;
            }
