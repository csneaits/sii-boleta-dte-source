...
class Container {
			...
			self::bind( TokenManager::class, fn() => new TokenManager( self::get( Api::class ), self::get( Settings::class ) ) );
						...
						self::bind( CheckoutFields::class, fn() => new CheckoutFields( self::get( Settings::class ) ) );
												self::bind( FolioManager::class, fn() => new FolioManager( self::get( Settings::class ) ) );
                                                                                                self::bind( QueueProcessor::class, fn() => new QueueProcessor( self::get( Api::class ), null, self::get( FolioManager::class ) ) );
                        self::bind( GenerateDtePage::class, fn() => new GenerateDtePage( self::get( Settings::class ), self::get( TokenManager::class ), self::get( Api::class ), self::get( DteEngine::class ), self::get( PdfGenerator::class ), self::get( FolioManager::class ), self::get( Queue::class ) ) );
												self::bind( ControlPanelPage::class, fn() => new ControlPanelPage( self::get( Settings::class ), self::get( FolioManager::class ), self::get( QueueProcessor::class ), self::get( Api::class ), self::get( TokenManager::class ) ) );
	...
	public static function get( string $id ) {
		// Ensure the container has been initialized. This makes the container
		// resilient in cases where callers invoke Container::get() before
		// Container::init() was explicitly called (for example during plugin
		// bootstrap in WordPress admin pages).
		if ( empty( self::$bindings ) ) {
			try {
				self::init();
			} catch ( \Throwable $e ) {
				// If init() fails, preserve original behaviour by throwing a
				// helpful InvalidArgumentException to the caller.
