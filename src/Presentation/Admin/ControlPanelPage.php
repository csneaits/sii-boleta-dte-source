...
class ControlPanelPage {
	...
	private array $notices = array();
            ...
            public function __construct( Settings $settings, FolioManager $folio_manager, QueueProcessor $processor, Api $api, TokenManager $token_manager ) {
                        ...
                        <div id="sii-notification-modal" class="sii-modal" style="display:none;">
                                ...
                                <div class="sii-modal-dialog">
                                        <div class="sii-modal-content">
                                                ...
                                                <pre id="sii-modal-details" style="display:none;white-space:pre-wrap;background:#f7f7fb;border-radius:6px;padding:12px;margin-top:12px;max-height:240px;overflow:auto;font-family:monospace;font-size:13px;"></pre>
                                                <div class="sii-modal-actions" style="display:flex;gap:8px;align-items:center;">
                                                        <button type="button" class="button" id="sii-modal-details-toggle" style="display:none;">
                                                                <?php echo esc_html__( 'Ver detalles', 'sii-boleta-dte' ); ?>
                                                        </button>
...
<?php AdminStyles::close_container(); ?>
        ...
        public function get_tab_content_html( string $tab ): string {
                $valid = array( 'logs', 'queue', 'metrics', 'cert', 'maintenance' );
...
private function render_queue(): void {
	...
	<div id="sii-control-queue-empty" class="sii-queue-empty<?php echo empty( $jobs ) ? '' : ' is-hidden'; ?>">
	        <div class="sii-queue-empty-card">
	                ...
	                <?php else : ?>
	                        <p class="sii-queue-empty-text"><?php echo esc_html__( 'La cola está vacía. Los documentos se agregarán automáticamente cuando se generen desde WooCommerce o se programen tareas automáticas.', 'sii-boleta-dte' ); ?></p>
	...
	private function verify_nonce( string $field, string $action ): bool {
		...
		if ( function_exists( 'wp_verify_nonce' ) ) {
			return (bool) \wp_verify_nonce( (string) $_POST[ $field ], $action );
                ...
                /**
                 * Add a notice for the control panel UI.
                 *
                 * @param string $message
                 * @param string $type 'success'|'error'|'warning'
                 * @param array<int,string> $details Optional array of detail lines to show in 'Ver detalles'
                 */
                private function add_notice( string $message, string $type = 'success', array $details = array() ): void {
                        $entry = array(
                                'type'    => 'error' === $type ? 'error' : 'success',
                                'message' => $message,
                        );
                        if ( ! empty( $details ) ) {
                                $entry['details'] = array_values( $details );
                        }
                        $this->notices[] = $entry;
                }
...
<?php foreach ( $jobs as $job ) :
        ...
        $show = in_array( $type, array( 'cleanup', 'tmp_cleanup', 'purge_temp' ), true ) || ! in_array( $type, array( 'dte', 'recibos' ), true );
        if ( ! $show ) {
                continue;
        }
        ?>
