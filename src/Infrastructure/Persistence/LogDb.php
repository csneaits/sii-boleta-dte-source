...
KEY env_status (environment, status)
        ...
        public static function add_entry( string $track_id, string $status, string $response, string $environment = '0', array $meta = array() ): void {
                // Do not persist incomplete "sent" rows without a track id — these
                // pollute the "DTE recientes" grid with entries that no sistema
                // externo podría rastrear. Upstream callers should always provide
                // a track_id when status is "sent".
                $normalized_status = strtolower( trim( (string) $status ) );
                if ( 'sent' === $normalized_status && '' === trim( (string) $track_id ) ) {
                        return;
                }
                global $wpdb;
        ...
        public static function get_logs_paginated( array $args = array() ): array {
                ...
                if ( is_object( $wpdb ) && method_exists( $wpdb, 'get_results' ) && method_exists( $wpdb, 'prepare' ) ) {
                        ...
                        // Exclude invalid 'sent'/'info' rows without track id
                        $clauses[] = "NOT (status = 'sent' AND (track_id IS NULL OR track_id = ''))";
                ...
                // Excluir filas inválidas: 'sent' sin track id
                $rows = array_filter(
                        $rows,
                        static function ( $row ) {
                                $st  = isset( $row['status'] ) ? strtolower( (string) $row['status'] ) : '';
