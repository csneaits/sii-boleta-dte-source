...
/**
 * Persistent queue storage used for pending jobs (e.g., DTE or recibos).
 *
...
KEY type (type)
	...
	self::$jobs[ $id ] = array(
        ...
        public static function find( int $id ): ?array {
                ...
                if ( is_object( $wpdb ) && method_exists( $wpdb, 'prepare' ) && method_exists( $wpdb, 'get_row' ) ) {
                        $sql  = $wpdb->prepare( 'SELECT id,type,payload,attempts,created_at FROM ' . self::table() . ' WHERE id = %d', $id );
                ...
                if ( isset( self::$jobs[ $id ] ) ) {
                        ...
                        return array(
                                ...
                                'available_at' => isset( $job['available_at'] ) ? (string) $job['available_at'] : '',
                        );
                }
	...
	public static function get_stats(): array {
        ...
        $stats = $wpdb->get_row( "
            SELECT 
                COUNT(*) as total,
                SUM(CASE WHEN attempts < 3 AND created_at <= NOW() THEN 1 ELSE 0 END) as pending,
