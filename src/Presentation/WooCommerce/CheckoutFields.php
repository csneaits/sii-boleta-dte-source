...
class CheckoutFields {
	...
	public function render_fields( $checkout ): void { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( function_exists( 'woocommerce_form_field' ) ) {
			...
			echo '<p class="form-row"><label>' . esc_html( $label ) . '</label><input type="text" name="billing_rut" /></p>';
                ...
                // Add observation field for boletas (types 39 and 41)
                if ( function_exists( 'woocommerce_form_field' ) ) {
                        woocommerce_form_field(
                                'sii_boleta_observacion',
                                array(
                                        'type'        => 'textarea',
                                        'label'       => __( 'Observación (opcional)', 'sii-boleta-dte' ),
                                        'placeholder' => __( 'Comentarios adicionales para la boleta', 'sii-boleta-dte' ),
                                        'required'    => false,
                        ...
                        echo '<p class="form-row form-row-wide"><label>' . esc_html__( 'Observación (opcional)', 'sii-boleta-dte' ) . '</label><textarea name="sii_boleta_observacion" rows="3" maxlength="200" placeholder="' . esc_attr__( 'Comentarios adicionales para la boleta', 'sii-boleta-dte' ) . '"></textarea></p>';
                }
	}
	...
	public function save_fields( int $order_id ): void {
                ...
                // Save observation field
                if ( isset( $_POST['sii_boleta_observacion'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
                        $observacion = function_exists( 'sanitize_textarea_field' ) 
                                ? sanitize_textarea_field( (string) $_POST['sii_boleta_observacion'] ) // phpcs:ignore WordPress.Security.NonceVerification.Missing
                                : sanitize_text_field( (string) $_POST['sii_boleta_observacion'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
                        if ( '' !== $observacion && function_exists( 'update_post_meta' ) ) {
                                update_post_meta( $order_id, '_sii_boleta_observacion', $observacion );
                        }
                }
	...
	public function display_admin_order_meta( $order ): void {
		...
		$order_id = $order->get_id();
                if ( function_exists( 'get_post_meta' ) ) {
                        ...
                        $observacion = get_post_meta( $order_id, '_sii_boleta_observacion', true );
                } else {
                        $type = $track_id = $pdf_key = $pdf_nonce = $observacion = '';
                }
		if ( $type ) {
		...
		if ( $observacion ) {
			echo '<p><strong>' . esc_html__( 'Observación', 'sii-boleta-dte' ) . ':</strong> ' . esc_html( (string) $observacion ) . '</p>';
		}
                $pdf_link = '';
