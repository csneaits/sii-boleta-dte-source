...
final class SII_Boleta_DTE {
	...
	public function init() {
		...
		// Instanciar la clase núcleo que configura todas las funcionalidades.
		// Proteger contra instalaciones donde el autoload de Composer no exista
		// y la clase alias no haya sido registrada todavía.
		// Ensure the dependency container is initialized before any components
		// attempt to resolve services. This avoids errors when pages are
		// constructed early in the admin bootstrap.
		if ( class_exists( '\Sii\BoletaDte\Infrastructure\Factory\Container' ) ) {
			\Sii\BoletaDte\Infrastructure\Factory\Container::init();
		}
		if ( class_exists( '\\Sii\\BoletaDte\\Infrastructure\\Plugin' ) ) {
			$core = new \Sii\BoletaDte\Infrastructure\Plugin();
		} elseif ( class_exists( '\\Sii\\BoletaDte\\Infrastructure\\WordPress\\Plugin' ) ) {
			// Usar la implementación específica de WordPress si está disponible.
			$core = new \Sii\BoletaDte\Infrastructure\WordPress\Plugin();
			// Registrar alias para compatibilidad con código que instancia la clase antigua
			if ( ! class_exists( '\\Sii\\BoletaDte\\Infrastructure\\Plugin' ) ) {
				class_alias( \Sii\BoletaDte\Infrastructure\WordPress\Plugin::class, 'Sii\\BoletaDte\\Infrastructure\\Plugin' );
			}
		} else {
			// Intentar requerir directamente el archivo conocido como último recurso.
			$maybe = SII_BOLETA_DTE_PATH . 'src/Infrastructure/WordPress/Plugin.php';
			if ( file_exists( $maybe ) ) {
				require_once $maybe;
				if ( class_exists( '\\Sii\\BoletaDte\\Infrastructure\\WordPress\\Plugin' ) ) {
					$core = new \Sii\BoletaDte\Infrastructure\WordPress\Plugin();
					class_alias( \Sii\BoletaDte\Infrastructure\WordPress\Plugin::class, 'Sii\\BoletaDte\\Infrastructure\\Plugin' );
				} else {
					// No se pudo localizar la clase núcleo; registrar un log y abortar inicialización.
					error_log( 'SII Boleta DTE: clase núcleo no encontrada después de require ' . $maybe );
					return;
				}
			} else {
				error_log( 'SII Boleta DTE: clase núcleo no encontrada y archivo esperado ausente: ' . $maybe );
				return;
			}
		}
	// Instanciar el manejador de cron para registrar el callback del evento
	new \Sii\BoletaDte\Infrastructure\Scheduling\Cron( $core->get_settings() );
	}
}
...
register_activation_hook( __FILE__, array( \Sii\BoletaDte\Infrastructure\Scheduling\Cron::class, 'activate' ) );
register_deactivation_hook( __FILE__, array( \Sii\BoletaDte\Infrastructure\Scheduling\Cron::class, 'deactivate' ) );
...
register_activation_hook( __FILE__, array( \Sii\BoletaDte\Infrastructure\Persistence\QueueDb::class, 'install' ) );
