												...
												<?php if ( is_array( $result ) && ! empty( $result['preview'] ) ) : ?>
																...
																<?php endif; ?>
                                                                                                <?php elseif ( is_array( $result ) && empty( $result['error'] ) ) : ?>
																							       ...
																							       $message  = '';
																							       // Prefer a UI-specific message when available (includes Track ID)
																							       if ( isset( $result['display_message'] ) && is_string( $result['display_message'] ) ) {
																								       $message = (string) $result['display_message'];
...
<option value="2" <?php selected( $current_dsc_ind_exe, '2', true ); ?>><?php esc_html_e( 'No facturable', 'sii-boleta-dte' ); ?></option>
	...
	public function process_post( array $post ): array {
		...
		if ( in_array( $tipo, array( 39, 41 ), true ) && '' === $rut ) {
				...
				if ( ! empty( $references ) ) {
						$data['Referencias'] = $references; }
                                ...
                                if ( ! $preview ) {
										...
										$reserved = null;
										if ( method_exists( $this->folio_manager, 'reserve_folio_temporarily' ) ) {
											$reserved = $this->folio_manager->reserve_folio_temporarily( $tipo );
										}
										$next = $reserved;
										// Fallback: some tests provide a FolioManager mock that implements get_next_folio
										// but not reserve_folio_temporarily. If reservation is not available or invalid,
										// attempt to obtain next folio directly.
										if ( ! is_numeric( $next ) || (int) $next <= 0 ) {
											if ( method_exists( $this->folio_manager, 'get_next_folio' ) ) {
												try {
                                ...
                                if ( $preview ) {
                                        $xml   = $this->engine->generate_dte_xml( $data, $tipo, true );
                                        $error = $engine_err( $xml );
                                        if ( null !== $error ) { return $error; }
                                        $pdf = $this->pdf->generate( (string) $xml );
                                } else {
                                        $env_now = $this->settings->get_environment();
                                        $retry_enabled = ( \Sii\BoletaDte\Infrastructure\WordPress\Settings::normalize_environment( $env_now ) === '2' );
						...
						if ( $this->folio_manager->mark_folio_used( $tipo, $folio ) ) {
							// Mantener el folio reservado para evitar duplicidad hasta que la cola confirme el envío.
							break; // reservado y finalizado
                                                ...
                                                $next_retry = $this->folio_manager->get_next_folio( $tipo, false );
                                                if ( function_exists( 'is_wp_error' ) && is_wp_error( $next_retry ) ) {
                                                        return array( 'error' => $next_retry->get_error_message() );
                                                }
                                                if ( $next_retry instanceof \WP_Error ) {
                                                        return array( 'error' => $next_retry->get_error_message() );
                                                }
                                                $folio = (int) $next_retry;
                                                if ( $folio <= 0 ) {
                                                        return array( 'error' => __( 'No hay folios disponibles para emitir este documento.', 'sii-boleta-dte' ) );
                                                }
                                                $data['Folio'] = $folio;
				...
				// En modo emisión ya reservamos el folio en el bloque anterior
				...
				$base_message = __( 'El documento fue encolado para su procesamiento. Aparecerá en "DTE recientes" en breve.', 'sii-boleta-dte' );
				// Include a visible Track ID in the queued message so UI tests and
				// integrations can always show an identifier regardless of where
				// the notice is rendered. Keep queued_track_id key for
				// compatibility with existing consumers.
				$message_with_track = sprintf( '%s %s', $base_message, sprintf( __( 'Track ID: %s', 'sii-boleta-dte' ), $temp_track_id ) );

				// Keep the canonical 'message' value for callers of process_post()
				// (some tests assert its exact string). Expose a 'display_message'
				// key that the UI rendering prefers when present so the admin page
				// can show the Track ID appended to the human-readable text.
				$result = array(
					'queued'           => true,
					'queued_track_id'  => $temp_track_id,
					'pdf_url'          => $pdf_url,
					'message'          => $base_message,
					'display_message'  => $message_with_track,
					'notice_type'      => 'info',
					'tipo'             => $tipo,
					'folio'            => $folio,
				);
