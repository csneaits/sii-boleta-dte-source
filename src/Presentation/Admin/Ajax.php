...
class Ajax {
    ...
    public function control_panel_data(): void {
        // No abortar con "0" para que el cliente siempre reciba JSON.
        $nonce = isset( $_POST['nonce'] ) ? (string) $_POST['nonce'] : ( isset( $_POST['_wpnonce'] ) ? (string) $_POST['_wpnonce'] : '' );
        if ( function_exists( 'wp_verify_nonce' ) && ! \wp_verify_nonce( $nonce, 'sii_boleta_control_panel' ) ) {
            \wp_send_json_error( array( 'message' => \__( 'Token expirado. Recarga la página.', 'sii-boleta-dte' ) ) );
        }
        if ( ! \current_user_can( 'manage_options' ) ) {
    ...
    $normalized_env = Settings::normalize_environment( (string) $environment );
        ...
        $logs        = $logs_data['rows'];
        // Incluir tanto los trabajos disponibles ahora como los programados
        // a futuro (reintentos) para que el panel muestre una imagen completa.
        $raw_jobs = QueueDb::get_pending_jobs( 100 );
        $all_jobs = QueueDb::get_all_jobs( 150 );
        if ( is_array( $all_jobs ) && ! empty( $all_jobs ) ) {
            ...
            if ( ! empty( $future ) ) {
                ...
                foreach ( $future as $j ) { $by_id[ (int) ( $j['id'] ?? 0 ) ] = $j; }
        ...
        \wp_send_json_success(
            ...
            $msg = function_exists( 'wp_strip_all_tags' ) ? wp_strip_all_tags( $e->getMessage() ) : strip_tags( $e->getMessage() );
            \wp_send_json_error( array( 'message' => $msg ?: __( 'Error interno del servidor.', 'sii-boleta-dte' ) ) );
        }
    }
    ...
    foreach ( array( 'filter_attempts', 'filter_age', 'filter_from', 'filter_to' ) as $key ) {
                if ( isset( $_POST[ $key ] ) ) {
    ...
    private function render_queue_rows( array $jobs ): array {
		...
		if ( ! $has_jobs ) {
			...
			foreach ( $jobs as $job ) {
                ...
                // Check if scheduled for a future time
                ...
                if ( $is_failed ) {
                    ...
                    echo '<br><small style="color:#856404;">' . esc_html__( 'Programado para reintento', 'sii-boleta-dte' ) . '</small>';
                }
                echo '</td>';
				echo '<td>';
	...
	private function render_queue_actions( array $job ): string {
		...
		$html .= '<form method="post" class="sii-inline-form" style="display:inline;margin:0;padding:0;">';
		...
		$html .= '<button type="submit" name="queue_action" value="requeue" class="button sii-queue-action sii-queue-action-sm" title="' . esc_attr__( 'Reintentar', 'sii-boleta-dte' ) . '">⟳</button>';
    ...
    /**
     * Returns a human readable label for a DTE numeric type.
     */
    private function format_dte_type_label( int $type ): string {
        if ( $type <= 0 ) { return 'DTE'; }
        $map = array(
            ...
            52 => __( 'Guía de despacho electrónica (52)', 'sii-boleta-dte' ),
    ...
    /**
     * Formats a MySQL datetime into a readable value.
     */
    private function format_log_date( string $datetime ): string {
        if ( '' === $datetime ) { return '—'; }
        $ts = strtotime( $datetime );
        if ( false === $ts ) { return $datetime; }
        if ( function_exists( 'date_i18n' ) ) {
            return (string) \date_i18n( 'Y-m-d H:i', $ts );
        }
