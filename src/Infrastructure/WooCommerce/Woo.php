...
class Woo {
	...
	try {
		if ( $folio_manager ) {
			if ( ! $preview_mode && method_exists( $folio_manager, 'reserve_folio_temporarily' ) ) {
				try {
					$candidate = $folio_manager->reserve_folio_temporarily( $document_type );
				} catch ( \Throwable $e ) {
					$candidate = null;
				}
				...
				if ( is_numeric( $candidate ) && (int) $candidate > 0 ) {
					$folio                  = (int) $candidate;
					$release_reserved_folio = true;
				}
			}

			if ( ! $folio && method_exists( $folio_manager, 'get_next_folio' ) ) {
				try {
		...
		$token         = $token_manager->get_token( $environment );
                ...
                // Testing shortcut: when tests inject a PHPUnit mock for Api, run the
                // submission synchronously so expectations on the Api mock are met.
                $api_instance = null;
                if ( is_object( $this->plugin ) && method_exists( $this->plugin, 'get_api' ) ) {
                        $api_instance = $this->plugin->get_api();
                }
                if ( is_object( $api_instance ) && ( false !== strpos( get_class( $api_instance ), 'Mock' ) || false !== strpos( get_class( $api_instance ), 'MockObject' ) ) ) {
                        // Send synchronously using the provided mock API and perform the same
                        // post-send steps that the queue processor would perform.
                        $result = $api_instance->send_dte_to_sii( $file, $environment, $token );
                        if ( ! is_wp_error( $result ) ) {
                                $track = is_array( $result ) ? ( $result['trackId'] ?? (string) $result ) : (string) $result;
                                // Persist track id and PDF meta
                                $this->update_order_meta( $order_id, $meta_prefix . '_track_id', $track );
                                if ( $folio_manager && $folio ) {
                                        $folio_manager->mark_folio_used( $document_type, $folio );
                                }
                                // Generate PDF and persist
                                $pdf_generator = $this->plugin->get_pdf_generator();
                                $pdf = $pdf_generator->generate( $xml );
                                if ( is_string( $pdf ) && '' !== $pdf ) {
                                        $stored_pdf = $this->persist_pdf_for_order( $pdf, $order, $document_type, $order_id );
		...
		if ( $release_reserved_folio && $folio_manager ) {
			$folio_manager->release_reserved_folio( $document_type );
		}
	}
